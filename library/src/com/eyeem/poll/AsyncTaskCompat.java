...
//   }
    ...
    * parallel. Starting {@link android.os.Build.VERSION_CODES#HONEYCOMB}, tasks
    * are back to being executed on a single thread to avoid common application
    * errors caused by parallel execution. If you truly want parallel execution,
    * you can use the {@link #executeOnExecutor} version of this method with
    ...
    * 
    * <p>
    * <em>Warning:</em> Allowing multiple tasks to run in parallel from a thread
    * pool is generally <em>not</em> what one wants, because the order of their
    * operation is not defined. For example, if these tasks are used to modify
    * any state in common (such as writing a file due to a button click), there
    * are no guarantees on the order of the modifications. Without careful work
    * it is possible in rare cases for the newer version of the data to be
    * over-written by an older one, leading to obscure data loss and stability
    * issues. Such changes are best executed in serial; to guarantee such work
    * is serialized regardless of platform version you can use this function
    ...
    * 
    * @param exec
    *           The executor to use. {@link #THREAD_POOL_EXECUTOR} is available
    *           as a convenient process-wide thread pool for tasks that are
    *           loosely coupled.
    * @param params
    *           The parameters of the task.
    * 
    ...
    * call to this method will trigger the execution of
